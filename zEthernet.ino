...
volatile int16_t keyaSteeringPosition = 0;
int16_t keyaRawAmps = 0;
volatile float keyaMotorAmps = 0.0;
//volatile float keyaMotorAmps =0; 
//uint8_t* dataPtr=0;
...
void KeyaBus_Receive()
{
    ...
    if (Keya_Bus.read(KeyaBusReceiveData))
    {
        ...
        if (KeyaBusReceiveData.id == 0x07000001)
        {
            ...
            //dataPtr=&KeyaBusReceiveData.buf[4];
            //keyaMotorAmps = *((float*)dataPtr);
            int16_t keyaRawAmps = (int16_t)((int16_t)KeyaBusReceiveData.buf[4] << 8 | (int16_t)KeyaBusReceiveData.buf[5]);
            keyaMotorAmps = keyaRawAmps;
            //Serial.print("Byte 4: "); Serial.print(KeyaBusReceiveData.buf[4], DEC);
            //Serial.print("Byte 5: "); Serial.print(KeyaBusReceiveData.buf[5], DEC);
            //Serial.print("Amps: "); Serial.println(MotorAmps, 4);
            ...
            if (error > abs(keyaCurrentSetSpeed) + 10)
            {
                ...
                {
                    Serial.print("Stop\t");
                    // working on this!!!! Serial.print(error)
