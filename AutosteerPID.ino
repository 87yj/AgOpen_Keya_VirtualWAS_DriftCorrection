/*
   ...
   For AgOpenGPS
   01 Feb 2022 updated Feb 2026 for Keya as Virtual WAS
...
extern volatile float keyaMotorAmps;
//extern volatile float keyaMotorAmps;
static int16_t lastKeyaEncoder = 0;
static long keyaEncoderTurns = 0;
static bool keyaEncoderInitialized = false;
...
float getKeyaContinuousDegrees(int16_t rawDeg)
{
    if (!keyaEncoderInitialized)
    {
        lastKeyaEncoder = rawDeg;
        keyaEncoderInitialized = true;
        return 0.0;
    }

    int16_t delta = rawDeg - lastKeyaEncoder;

    if (delta > 180) keyaEncoderTurns--;
    else if (delta < -180) keyaEncoderTurns++;

    lastKeyaEncoder = rawDeg;

    return (keyaEncoderTurns * 360.0f) + rawDeg;
}
...
void autosteerSetup()
{
 ...
 //   Serial.print("KeyaDetected: ");
 //   Serial.print(keyaDetected);
    Serial.print("   SingleInputWAS: ");
    Serial.println(steerConfig.SingleInputWAS);
...
//for debugging of Keya Virtual WAS, eliminate when in production
...
void autosteerLoop()
{
  ...
  //if (keyaSteeringPosition != last)
  //{
  //  Serial.println(keyaSteeringPosition);
  //  last = keyaSteeringPosition;
  //}
  ...
  if (currentTime - autsteerLastTime >= LOOP_TIME)
  {
    ...
    if (guidanceStatusChanged)
    {
        if (guidanceStatus == 1)    //Must have changed Off >> On
        {
            currentState = 0;
            steerSwitch = 0;
            //Serial.println("Guidance Status Change");
        }
    }
    ...
    if (steerConfig.CurrentSensor)
    {
        if (keyaDetected)
        {
            //Serial.print(" CurrentSEnsor "); Serial.print(steerConfig.CurrentSensor);
            //Serial.print("  Sensor Reading: "); Serial.print(sensorReading);
            //Serial.print("  Motor Amps: "); Serial.print(keyaMotorAmps);
            //Serial.print("PulseCountMax: "); Serial.println(steerConfig.PulseCountMax);

            
            if (abs(sensorReading) >= steerConfig.PulseCountMax/25)
            {
                steerSwitch = 1;
                currentState = 1;
                previous = 0;
                Serial.println("Soft limit for Motor Over Current in Amps");
                Serial.print(" CurrentSEnsor "); Serial.print(steerConfig.CurrentSensor);
                Serial.print("  Sensor Reading: "); Serial.print(sensorReading);
                Serial.print("  Motor Amps: "); Serial.println(keyaMotorAmps);
                Serial.print("PulseCountMax: ");
                Serial.print(steerConfig.PulseCountMax);
                Serial.print("  Limit: ");
                Serial.print(steerConfig.PulseCountMax/25);
                Serial.print("  Amps: ");
                Serial.println(keyaMotorAmps);
            }
        }
        else
        {
...
// use Keya encoder as simulated ADS1115 WAS
    if (keyaDetected && !steerConfig.SingleInputWAS)
    {
        ...
        //Serial.print(" MotorDeg: "); Serial.print(motorDeg);
        //Serial.print(" TireDeg: "); Serial.print(TireDeg);
        //Serial.print("  steerAngleActual: "); Serial.print(steerAngleActual);
